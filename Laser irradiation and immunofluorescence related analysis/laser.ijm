...
// Set parameters: base_dir is the path where laser images are stored; roi_dir is the path to laser ROI files corresponding to each cell, which need to be manually selected in Fiji (ImageJ) and saved beforehand; Threshold_green is the defined threshold
