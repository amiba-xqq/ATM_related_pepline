...
folder_path <- "C:/Users/13103/Desktop/niechen 20250409/iCas-lib c1_merged tif/"
...
repeat{
  ...
  # 查找当前工作目录下所有以"_red_foci.csv"结尾的CSV文件
  # Search for all CSV files ending with "_red_foci.csv" in the current working directory
  file_list <- list.files(pattern = "_red_foci.csv$")
  ...
  write.csv(merged_data, file = "red_foci_merge.csv", row.names = FALSE) 
}

# 最后得到的green_foci_merge.csv文件展示了合并后各组green通道foci数目；red_foci_merge.csv文件展示了合并后各组green通道foci数目。
# The final green_foci_merge.csv file shows the merged number of green channel foci for each group; the red_foci_merge.csv file shows the merged number of red channel foci for each group.
