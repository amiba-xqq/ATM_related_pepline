...
for (image=0; image < n_images; image++) {
  ...
  // 下面中c1是red通道，c3是blue通道; merged_image0指C=0结尾的图片，merged_image1指C=1结尾的图片
  // Below: c1 is the red channel, c3 is the blue channel; merged_image0 refers to images ending with C=0, merged_image1 refers to those ending with C=1
  // 更改merged_image0/1的位置，使得它们和c1/c3的通道颜色相同
  // Modify the sequence of merged_image0, and 1 so that their colors align with those of c1, and c3. For instance, merged_image0 corresponds to images ending with C=0 (DAPI channel), merged_image1 to C=1 (red channel)
