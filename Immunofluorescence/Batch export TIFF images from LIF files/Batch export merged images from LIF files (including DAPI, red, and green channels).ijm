// 第5行和第40行是唯一需要改的地方
// Lines 5 and 40 are the only ones that need to be modified
...
for (image=0; image < n_images; image++) {
  ...
  selectImage(figure_m+3);
  merged_image2 = getTitle();

  // 下面中c1是red通道，c2是green通道，c3是blue通道;merged_image0指C=0结尾的图片，merged_image1指C=1结尾的图片，merged_image2指C=2结尾的图片
  // Below: c1 is the red channel, c2 is the green channel, c3 is the blue channel; merged_image0 refers to images ending with C=0, merged_image1 refers to those ending with C=1, and merged_image2 refers to those ending with C=2
  // 更改merged_image0/1/2的顺序，使得它们和c1/c2/c3的通道颜色相同；比如下面指C=0结尾图片是DAPI通道，C=1结尾图片是green通道，C=2结尾图片是red通道
  // Modify the sequence of merged_image0, 1, and 2 so that their colors align with those of c1, c2, and c3. For instance, merged_image0 corresponds to images ending with C=0 (DAPI channel), merged_image1 to C=1 (green channel), and merged_image2 to C=2 (red channel)
  run("Merge Channels...", "c1=[" + merged_image2 + "] c2=[" + merged_image1 + "] c3=[" + merged_image0 + "] keep");
